...
public class CompressUtil {
    ...
    public static void zipCompress(String sourPath, String tarPath, String format) {
        ...
        try {
            ...
            // 压缩输出流
            tarArchiveInputStream = new TarArchiveInputStream(fileOutputStream);
            // 打包压缩文件
     ...
     * @throws Exception
     */
    private static void generateFile(TarArchiveOutputStream out, File file, String dir) throws Exception {
        // 当前的是文件夹，则进行一步处理
        if (file.isDirectory()) {
            //得到文件列表信息
...
//            out.putNextEntry(new ZipEntry(dir + "/"));
            ...
            // 标记要打包的条目
//            out.putArchiveEntry(new ZipEntry(dir));
            // 进行写操作
            int len = 0;
